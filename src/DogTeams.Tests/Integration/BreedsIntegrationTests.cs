...
public class BreedsIntegrationTests : IClassFixture<AppHostFixture>
{
    ...
    {
        var response = await _client.GetAsync("/breeds");
    ...
    [Fact(Skip = "GetBreed endpoint returns 501 (not yet implemented)")]
    public async Task GetBreed_ById_ReturnsBreed()
    {
        // Fetch the breed list and retrieve the first breed by id
        var listResponse = await _client.GetAsync("/breeds");
        ...
        var getResponse = await _client.GetAsync($"/breeds/{firstId}");
